  @extends('layouts.app')

  @section('content')
      <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
          <div class="container-fluid py-2">
              <div class="row">
                  <div class="col-12">
                      <div class="card my-4">
                          <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                              <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3">
                                  <h6 class="text-white text-capitalize ps-3">Candidates table</h6>
                              </div>
                          </div>
                          <div class="card-body px-0 pb-2">
                              <div class="table-responsive p-0">
                                  <table class="table align-items-center mb-0">
                                      <thead>
                                          <tr>
                                              <th
                                                  class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                                  Prénom
                                              </th>
                                              <th
                                                  class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                                  Nom
                                              </th>
                                              <th
                                                  class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                                  Pipeline
                                              </th>
                                              <th
                                                  class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                                  Notation
                                              </th>
                                              <th
                                                  class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                                  
                                              </th>
                                          </tr>
                                      </thead>
                                      <tbody>
                                          @foreach ($candidates as $candidate)
                                              <tr>
                                                  <td>
                                                      <span class="text-xs font-weight-bold">{{ $candidate->first_name }}
                                                      </span>
                                                  </td>
                                                  <td>
                                                      <span class="text-xs font-weight-bold">{{ $candidate->last_name }}
                                                      </span>
                                                  </td>
                                                  <td>
                                                    <i class="bg-info"></i>
                                                      <span class="text-xs font-weight-bold">{{ $candidate->recruitment_pipeline }}
                                                      </span>
                                                  </td>
                                                  <td>
                                                      <span class="text-xs font-weight-bold">{{ $candidate->notation }}
                                                      </span>
                                                  </td>
                                                  <td>
                                                      <span class="text-xs font-weight-bold">Afficher plus
                                                      </span>
                                                  </td>
                                             </tr>
                                          @endforeach
                                      </tbody>
                                  </table>
                              </div>
                          </div>
                      </div>
                      <a class="btn bg-gradient-dark mb-0" href="{{ route('candidates.create') }}"><i
                              class="material-symbols-rounded text-sm">add</i>&nbsp;&nbsp;Ajouter un candidate</a>
                  </div>
              </div>
          </div>
      </main>
  @endsection
